// Wir checken bei jedem Item das ins Projekt kommt den MediaPath
// Ist Material dabei, dass noch nicht auf einem Share-Ordner liegt wird der/die User*in benachrichtigt.
...
function onActiveSequenceChanged() {
    ...
    var JSONResult = CheckPathOfTimelineClips(app.project.activeSequence)
    var result = JSON.parse(JSONResult)
    
    if (result.Anzahl > 0) {
        for (var i = 0; i < result.Names.length; i++) {
        alert("Das folgende Asset: " + result.Names[i] + " liegt nicht auf einem Share-Laufwerk. Bitte informieren Sie den/die Postpro-Expert*in.")
        }
    }
...
function CheckPathOfTimelineClips(mysequence) {
    arrClipPaths.length = 0
    arrClipNames.length = 0
    //PathObj.Anzahl = 0
    if (mysequence) {
